...
# If you already have these constants, keep yours and remove these.
REQUESTS_FILE = "requests.json"
COMMENTS_FILE = "comments.json"

# If you already have choose_export_dir(), keep yours and remove this fallback.
def _fallback_export_dir() -> Path:
    p = Path.cwd() / "exports"
    p.mkdir(parents=True, exist_ok=True)
    return p

# ---- Session flags
if "has_saved_json" not in st.session_state:
    st.session_state.has_saved_json = False  # overlay stays until this becomes True
if "saved_json_paths" not in st.session_state:
    st.session_state.saved_json_paths = {}   # optional: store where we saved
if "save_guard_opened" not in st.session_state:
    st.session_state.save_guard_opened = False  # internal

def mark_json_saved(**paths):
    """
    Call this at the end of your own save/export function to dismiss the overlay.
    Example: mark_json_saved(requests="/path/to/requests.json", comments="/path/to/comments.json")
    """
    st.session_state.has_saved_json = True
    if paths:
        st.session_state.saved_json_paths.update(paths)
    st.toast("JSON saved. You're good to go ✅", icon="✅")

def _export_all_json_now():
    """
    Default action for the modal's 'Save JSON now' button.
    Writes requests/comments from session_state to disk, then dismisses the overlay.
    If you already have your own exporter, just call that and then mark_json_saved().
    """
    # Prefer your own choose_export_dir() if present
    export_dir = None
    if "choose_export_dir" in globals() and callable(globals()["choose_export_dir"]):
        try:
            export_dir = globals()["choose_export_dir"]()
        except Exception:
            export_dir = None
    if export_dir is None:
        export_dir = _fallback_export_dir()

    saved = {}
    # Save requests
    if "requests" in st.session_state and st.session_state["requests"] is not None:
        rp = export_dir / REQUESTS_FILE
        rp.write_text(json.dumps(st.session_state["requests"], ensure_ascii=False, indent=2))
        saved["requests"] = str(rp)
    # Save comments
    if "comments" in st.session_state and st.session_state["comments"] is not None:
        cp = export_dir / COMMENTS_FILE
        cp.write_text(json.dumps(st.session_state["comments"], ensure_ascii=False, indent=2))
        saved["comments"] = str(cp)

    # Mark and close
    mark_json_saved(**saved)
    st.rerun()

@st.dialog("⚠️ Don’t forget to save the JSON", width="large")
def _save_guard_modal():
    st.markdown(
        """
        **Before using the app**, save/export your current data to a JSON file so nothing is lost if the app restarts.

        - This saves the current **Requests** and **Comments** from memory to disk.
        - The overlay will close automatically after a successful save.
        """)
    st.button("💾 Save JSON now", use_container_width=True, on_click=_export_all_json_now)
    st.caption("This reminder appears on every page and only disappears after saving.")

# ---- Gate the entire app until saved
if not st.session_state.has_saved_json:
    # Open modal on first render and keep gating until the save flag is set
    _save_guard_modal()
    st.stop()
